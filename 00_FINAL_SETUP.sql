...
-- Otomatik Profil Oluşturma Trigger'ı (auth.users -> public.profiles)
...
-- =========================================================
-- 2. KATEGORİLER (sections) VE TIKLAMA ANALİTİĞİ
-- =========================================================
...
CREATE POLICY "Giriş yapmış tüm kullanıcılar aktif bölümleri okuyabilir" ON public.sections FOR SELECT USING (auth.role() = 'authenticated' AND (is_active = true OR public.is_admin()));
DROP POLICY IF EXISTS "Sadece adminler bölüm ekleyebilir" ON public.sections;
CREATE POLICY "Sadece adminler bölüm ekleyebilir" ON public.sections FOR INSERT WITH CHECK (public.is_admin());
DROP POLICY IF EXISTS "Sadece adminler bölüm güncelleyebilir" ON public.sections;
CREATE POLICY "Sadece adminler bölüm güncelleyebilir" ON public.sections FOR UPDATE USING (public.is_admin());
DROP POLICY IF EXISTS "Sadece adminler bölüm silebilir" ON public.sections;
CREATE POLICY "Sadece adminler bölüm silebilir" ON public.sections FOR DELETE USING (public.is_admin());
...
CREATE POLICY "Giriş yapmış kullanıcılar kendi tıklama kaydını oluşturabilir" ON public.section_clicks FOR INSERT WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "Sadece adminler tüm tıklama kayıtlarını okuyabilir" ON public.section_clicks;
CREATE POLICY "Sadece adminler tüm tıklama kayıtlarını okuyabilir" ON public.section_clicks FOR SELECT USING (public.is_admin());

-- =========================================================
-- 3. ÜRÜNLER (products) VE ÇOKLU VİDEOLAR (product_videos)
-- =========================================================
...
DROP POLICY IF EXISTS "Müşteriler yayınlanmış ürünleri okuyabilir" ON public.products;
CREATE POLICY "Müşteriler yayınlanmış ürünleri okuyabilir" ON public.products FOR SELECT USING (auth.role() = 'authenticated' AND (is_published = true OR public.is_admin()));
...
CREATE POLICY "Adminler güncelleyebilir" ON public.products FOR UPDATE USING (public.is_admin());
...
-- =========================================================
-- 4. SİTE AYARLARI (site_settings) VE DÖKÜMAN LİNKLERİ
-- =========================================================
...
-- =========================================================
-- 6. STORAGE BUCKETS (DEPOLAMA ALANLARI) VE ÖRNEK VERİLER
-- =========================================================
...
-- 9. SIDREX GER�EK OR�J�NAL �R�NLER� (SHOPIFY CDN G�RSELLER� �LE)
INSERT INTO public.products (section_id, title, slug, description, specs, video_type, video_url, thumbnail_url, is_published)
VALUES 
  -- �zel Takviyeler (Electrolyte Balance & Slm-X)
  (
    'c7000000-0000-0000-0000-000000000007',
    'Electrolyte Balance',
    'electrolyte-balance',
    'Sidrex� Electrolyte Balance; pembe Himalaya deniz tuzu, 5�li elektrolit kompleksi, C, B6 ve B12 vitaminleri ile zenginle�tirildi. Bu �zel form�l; modern bilimin g�c�n� lezzetli ve pratik bir i�ecekle bulu�turuyor.',
    '{"Form": "Stick Sa�e", "Gramaj": "30 Sa�e", "�zellikler": "�ekersiz, Vegan, Gl�tensiz, Koruyucu ��ermez", "Fiyat": "549.00 TL", "SKU": "152-SDRX-ELT", "Kullan�m �ekli": "G�nde 1 stick sa�eyi 500 mL su ile kar��t�rarak t�ketiniz."}'::jsonb,
    'youtube',
  ...
  (
    'c7000000-0000-0000-0000-000000000007',
    'Slm-X | Takviye Edici G�da',
    'slm-x',
    'Sidrex� Slm-X; bromelain, CLA, L-karnitin, in�lin ve ye�il �ay ekstresi ba�ta olmak �zere 7 bile�enli form�l�yle geli�tirilmi�, ananas aromal� sa�e takviyedir.',
    '{"Form": "Sa�e", "Gramaj": "30 Sa�e", "�zellikler": "Yapay Boya Yok, Koruyucusuz, Ananas Aromal�", "Fiyat": "1.890.00 TL", "SKU": "153-SDRX-SLMX", "Kullan�m �ekli": "G�nde 1 sa�e suda ��z�nd�r�lerek t�ketilir."}'::jsonb,
    'vimeo',
  ...
  (
    ...
    'b12-complex-b12-b1-b2-b6-ve-folik-asit',
    'B12, B1, B2, B6 vitaminleri ve aktif folik asit i�eri�iyle enerji olu�um metabolizmas�na katk�da bulunur, yorgunluk ve bitkinli�i azaltmaya yard�mc� olur.',
    '{"Form": "Damla / Sprey", "Gramaj": "30 ml", "�zellikler": "�ekersiz, Yapay Boya ��ermez", "Fiyat": "500.00 TL", "SKU": "152-SDRX-B12", "Kullan�m �ekli": "G�nde 1 puff dil alt�na p�sk�rt�l�r."}'::jsonb,
    'youtube',
    'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
  ...
  (
    'c4000000-0000-0000-0000-000000000004',
    'Lipo Iron Complex | Takviye Edici G�da',
    'lipo-iron-complex',
    'Lipozomal mikroenkaps�le Lipofer� demir, C vitamini, aktif folat ve B vitaminleri ile mide hassasiyeti ve kab�zl�k yapmayan y�ksek emilimli demir.',
    '{"Form": "Kaps�l", "Gramaj": "30 Kaps�l", "�zellikler": "Vegan, TiO2 ��ermez, G�S Hassasiyeti Yapmaz", "Fiyat": "900.00 TL", "SKU": "153-SDRX-LIPO", "Kullan�m �ekli": "G�nde 1 kaps�l a� karn�na su ile."}'::jsonb,
    'youtube',
    'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
  ...
  (
    ...
    'b12-complex-kids',
    '�ocuklar�n zihinsel ve fiziksel geli�imini desteklemek �zere geli�tirilmi� B12, B1, B2, B6 vitaminleri ve folik asit kompleksi.',
    '{"Form": "Damla", "Gramaj": "30 ml", "�zellikler": "�ekersiz, �ocuklara �zel Dozaj", "Fiyat": "490.00 TL", "SKU": "152-SDRX-B12KIDS", "Kullan�m �ekli": "�ocuklar i�in g�nde 1 damla/puff."}'::jsonb,
    'vimeo',
    'https://vimeo.com/76979871',
  ...
  (
    ...
    'lipo-iron-kids',
    '�ocuklar�n g�nl�k demir ihtiyac�n� kar��layan, di� lekelenmesi ve tat rahats�zl��� yapmayan lezzetli lipozomal damla formu.',
    '{"Form": "Damla", "Gramaj": "30 ml", "�zellikler": "Di� Leke Yapmaz, �ocuk G�venlikli Kapak", "Fiyat": "650.00 TL", "SKU": "152-SDRX-LPKIDS", "Kullan�m �ekli": "G�nde 1 ml damla do�rudan veya meyve suyuna eklenir."}'::jsonb,
    'youtube',
    'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
  ...
  (
    'c5000000-0000-0000-0000-000000000005',
    '�ocuk Mevsim Ge�i�i Seti',
    'cocuk-mevsim-gecis-seti',
    'Mevsim de�i�ikliklerinde �ocuklar�n direncini korumak i�in tasarlanm�� Imuntus Kids ve D3K2 takviye seti.',
    '{"Form": "Set", "��erik": "Imuntus Kids + Vitamin D3K2 Kids", "�zellikler": "Avantajl� Paket, %10 �ndirimli", "Fiyat": "1.149.00 TL", "SKU": "SET-ALERJISET", "Kullan�m �ekli": "G�nl�k 1 sa�e ve 1 damla."}'::jsonb,
    'youtube',
    'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
  ...
  (
    ...
    'Happy Tummies Set',
    'happy-tummies-set',
    '�ocuklarda sindirim ve mide konforu sa�layan probiyotik lif ve multivitamin ikili takviye paketi.',
    '{"Form": "Set", "��erik": "Colovita Kids + B12 Complex Kids", "�zellikler": "Sindirim Dostu, Do�al Tat", "Fiyat": "1.265.00 TL", "SKU": "SET-DIGESTSETKIDS", "Kullan�m �ekli": "G�nde 1 sa�e ve 1 damla."}'::jsonb,
    'vimeo',
    'https://vimeo.com/76979871',
  ...
  (
    ...
    '�muntus | Bitkisel Takviye Edici G�da',
    'imuntus-bitkisel',
    'Zahter, zencefil, ard��, karaba� otu, ��rek otu ya��, C vitamini ve �inko i�eren Anadolu bitkileri destekli �urup.',
    '{"Form": "�urup", "Gramaj": "150 ml", "�zellikler": "�ekersiz, Yapay Boya ve Koruyucu ��ermez", "Fiyat": "500.00 TL", "SKU": "153-SDRX-IM", "Kullan�m �ekli": "G�nde 1 �l�ek (10 ml) yemekten sonra."}'::jsonb,
    'youtube',
    'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
  ...
  (
    ...
    'collagen-glow-complex',
    'Tip 1 & Tip 3 hidrolize kolajen peptidleri, hyaluronik asit, C vitamini ve biyotin ile cilt parlakl��� ve esnekli�i i�in �zel form�l.',
    '{"Form": "Sa�e", "Gramaj": "30 Sa�e", "�zellikler": "�ekersiz, Gl�tensiz, Tatland�r�c� ��ermez", "Fiyat": "1.450.00 TL", "SKU": "SX-COL-GLOW", "Kullan�m �ekli": "G�nde 1 sa�eyi 200 ml suda ��zd�rerek t�ketiniz."}'::jsonb,
    'youtube',
    'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
  ...
  (
    ...
    'milk-thistle-complex',
    'Devedikeni ekstrat� (Silymarin), enginar ve karahindiba k�k� ile karaci�er detoksu ve sindirim sa�l��� takviyesi.',
    '{"Form": "Kaps�l", "Gramaj": "60 Bitkisel Kaps�l", "�zellikler": "Vegan, GDO ��ermez", "Fiyat": "750.00 TL", "SKU": "SX-MILK-THISTLE", "Kullan�m �ekli": "G�nde 1-2 kaps�l yemeklerden �nce."}'::jsonb,
    'youtube',
    'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
